			...
			class form{
				...
				*/
				...
				public function href($titre){
						return $this->retourAlaligne('<a href="#" id="lieninscription">'.$titre.'</a>');
				}
				...
				*/
				public function submit($nom,$libelle){
						if($libelle != "se connecter"){
								return $this->retourAlaligne('<button type="submit" name"'.$nom.'" class="btn btn-primary" id="submit">'.$libelle.'</button>');
						}else{
								return $this->retourAlaligne('<button type="submit" name"'.$nom.'" class="btn btn-primary" id="submit">'.$libelle.'</button>');
						}
				...
				*@return on retourn le champs bien preparer et bien construit
				*/
				public function input($nom,$libelle,$type){
						if($type == "password"){
								return '<label id="pass">'.$libelle.'</label>'.$this->retourAlaligne('<input type="'.$type.'" name="'.$nom.'" class="form-control" placeholder="********" id="password">');
						}else{
									return '<label id="nom">'.$libelle.'</label>'.$this->retourAlaligne('<input type="'.$type.'" name="'.$nom.'" class="form-control" placeholder="nom d utilisateur"  id="username">');
						}
						}
